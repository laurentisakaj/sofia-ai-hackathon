  ...
  switch (name) {
    ...
    case 'create_personalized_quotation': {
      ...
      if (quotationResult.success) {
        ...
        // Schedule 24h quotation follow-up WhatsApp message (cancel any existing first to prevent duplicates)
        ...
        if (guestPhone && quotationResult.booking_link) {
          await cancelByPhoneAndType(guestPhone, 'quotation_followup').catch(() => {});
    ...
    case 'show_visual_assist': {
      ...
      return { displayed: true, message: `Visual assist card "${args.title}" shown on screen. Do not repeat the content verbally — the guest can see it.` };
    }
