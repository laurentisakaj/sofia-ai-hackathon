...
// Base URL for links
const BASE_URL = process.env.BASE_URL || "https://sofia-ai-942607221166.europe-west1.run.app";
...
const GOOGLE_API_REFERER = 'https://sofia-ai-942607221166.europe-west1.run.app/';
...
setInterval(() => {
  ...
  // Cleanup old wa_sessions history files (same TTL as in-memory sessions)
  const waSessionsDir = path.join(DATA_DIR, 'wa_sessions');
  fs.promises.readdir(waSessionsDir).then(files => {
    for (const file of files) {
      if (!file.endsWith('.json')) continue;
      const filePath = path.join(waSessionsDir, file);
      fs.promises.stat(filePath).then(stat => {
        if (now - stat.mtimeMs > WHATSAPP_SESSION_TTL) {
